...
<body>
<body id="body-pd" style="background-image: url('/OnlinefruitDelivery/img/background1.png');">
    ...
    <div class="container my-3" id="cont">
        ...
        <?php
            $id = $_GET['catid'];
            $sql = "SELECT * FROM `fruit` WHERE fruitCategorieId = $id";
            $result = mysqli_query($conn, $sql);
            $noResult = true;
            while($row = mysqli_fetch_assoc($result)){
                $noResult = false;
                ...
                $fruitPrice = $row['fruitPrice'];
                $fruitDesc = $row['fruitDesc'];
            
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                        <div class="card" style="width: 18rem;">
                            <img src="img/fruit-'.$fruitId. '.jpg" class="card-img-top" alt="image for this fruit" width="249px" height="270px">
                            <div class="card-body">
                                <h5 class="card-title">' . substr($fruitName, 0, 20). '...</h5>
                                <h6 style="color: #ff0000">Rs. '.$fruitPrice. '/-</h6>
                                <p class="card-text">' . substr($fruitDesc, 0, 29). '...</p>   
                                <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `itemQuantity` FROM `viewcart` WHERE fruitId = '$fruitId' AND `userId`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$fruitId. '">
                            ...
                            echo '</form>                            
                                <a href="viewfruit.php?fruitid=' . $fruitId . '" class="mx-2"><button class="btn btn-primary">Quick View</button></a> 
                                </div>
                            </div>
                        </div>
                    </div>';
